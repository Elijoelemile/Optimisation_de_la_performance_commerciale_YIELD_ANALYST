"""
pipeline_etl.py — Orchestrateur ETL NG Travel (E → T → L)
==========================================================

C'est ici que la « variable mémoire » prend tout son sens : les trois étapes
s'exécutent dans le MÊME processus Python, donc le DataFrame produit par
Transformation est passé DIRECTEMENT à Load, sans jamais toucher le disque
entre les deux.
...
--------------------------------------------------------
    python -m pipeline_etl.pipeline_etl "/Users/moi/Downloads/base_de_données_NG_Travel.xlsx" \
        --nom ng_travel_saison_2025_2026

    # Ou en cherchant le fichier dans un dossier :
    python -m pipeline_etl.pipeline_etl "/Users/moi/Downloads" --pattern NG_Travel \
        --nom ng_travel_v1
"""
...
def _cli():
    parser = argparse.ArgumentParser(description="Pipeline ETL NG Travel (E→T→L).")
